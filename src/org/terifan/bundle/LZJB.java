...
public class LZJB
{
	...
	public byte[] compress(byte[] aSrcBuffer, int aSrcLen) throws IOException
	{
		...
		while (src < aSrcLen)
		{
			...
			{
				work.write(0xff & aSrcBuffer[src++]);
				continue;
			}
			...
			{
				int dist = mWindowOffset + src - mRefs[hash][i];
				int cpy = mRefs[hash][i];

				if (dist >= 0 && dist < WINDOW_SIZE && cpy + MATCH_MIN < mWindowOffset + src)
				{
					int mlen = 0;
					for (; src + mlen < aSrcLen && mlen < MATCH_MAX; mlen++)
					{
						if (aSrcBuffer[src + mlen] != mWindow[cpy + mlen])
						{
							break;
						}
					}
			...
			mRefs[hash][0] = mWindowOffset + src;
